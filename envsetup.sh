...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^purity_") ; then
       PURITY_BUILD=$(echo -n $1 | sed -e 's/^purity_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $PURITY_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       PURITY_BUILD=
    fi
    export PURITY_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${PURITY_DEVICES_ONLY}" != "z" ]; then
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var PURITY_VERSION)
        ZIPFILE=purity-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell getprop ro.purity.device | grep -q "$PURITY_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $PURITY_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.purity.device | grep -q "$PURITY_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/purity/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
